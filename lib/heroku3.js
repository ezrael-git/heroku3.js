...
class heroku_conn {
  ...
  static async build (key) {
    ...
    let account = await Account.build(key); // to access all account info returned by the api you can do <Account>.raw
  ...
  async apps (sort_by=null) {
    ...
    apps_raw.forEach( function (raw_app) {
      let converted = new App(key,raw_app.name,raw_app.id,raw_app.created_at,raw_app.maintenance,raw_app.git_url,raw_app);
